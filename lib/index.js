      ...
      try {
        ...
        self.sendspinPlayer = new SendspinClient({
          ...
          onVolumeChange: (volume) => {
            // Server requested a volume change — apply it to Volumio's mixer.
            // commandRouter.volumiosetvolume accepts an integer 0-100 and routes
            // through the user-configured mixer (hardware/software/none).
            self.logger.info(`${self.loggerPrefix} Setting Volumio volume to ${volume}`);
            try {
              self.commandRouter.volumiosetvolume(volume);
            } catch (e) {
              self.logger.error(`${self.loggerPrefix} volumiosetvolume(${volume}) failed:`, e);
            }
          },
          onMuteChange: (muted) => {
            // Server requested mute/unmute — same API accepts 'mute'/'unmute' string
            self.logger.info(`${self.loggerPrefix} Setting Volumio mute=${muted}`);
            try {
              self.commandRouter.volumiosetvolume(muted ? 'mute' : 'unmute');
            } catch (e) {
              self.logger.error(`${self.loggerPrefix} volumiosetvolume(${muted ? 'mute' : 'unmute'}) failed:`, e);
            }
          },
