...
class SendspinClient {
  constructor(config = {}) {
    ...
    this.onVolumeChange = config.onVolumeChange || null; // Callback when server requests volume change (0-100)
    this.onMuteChange = config.onMuteChange || null; // Callback when server requests mute change (boolean)
        ...
        case 'server/command':
          ...
          if (payload.player) {
            ...
            if (cmd === 'volume' && payload.player.volume !== undefined) {
              ...
              // Notify controller so it can apply the volume to the underlying mixer
              if (this.onVolumeChange) {
                try {
                  this.onVolumeChange(payload.player.volume);
                } catch (e) {
                  this.config.logger.error('[SendspinClient] onVolumeChange callback threw:', e);
                }
              }
              // Send updated state
              ...
              // Notify controller so it can apply mute to the underlying mixer
              if (this.onMuteChange) {
                try {
                  this.onMuteChange(payload.player.mute);
                } catch (e) {
                  this.config.logger.error('[SendspinClient] onMuteChange callback threw:', e);
                }
              }
              // Send updated state
              this.sendState('synchronized', this.config.volume, this.config.muted);
